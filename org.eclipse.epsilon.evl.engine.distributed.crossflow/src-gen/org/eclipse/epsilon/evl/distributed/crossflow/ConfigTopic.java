...
public class ConfigTopic extends JobStream<Config> {
		
	public ConfigTopic(Workflow workflow, boolean enablePrefetch) throws Exception {
		super(workflow);
		
		ActiveMQDestination postQ;
			pre.put("JobDistributor", (ActiveMQDestination) session.createQueue("ConfigTopicPre.JobDistributor." + workflow.getInstanceId()));
			destination.put("JobDistributor", (ActiveMQDestination) session.createQueue("ConfigTopicDestination.JobDistributor." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createTopic("ConfigTopicPost.JobDistributor." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
			post.put("JobDistributor", postQ);			
			pre.put("Processing", (ActiveMQDestination) session.createQueue("ConfigTopicPre.Processing." + workflow.getInstanceId()));
			destination.put("Processing", (ActiveMQDestination) session.createQueue("ConfigTopicDestination.Processing." + workflow.getInstanceId()));
			postQ = (ActiveMQDestination) session.createTopic("ConfigTopicPost.Processing." + workflow.getInstanceId()
					+ (enablePrefetch?"":"?consumer.prefetchSize=1"));		
	...
	public void addConsumer(ConfigTopicConsumer consumer, String consumerId) throws Exception {
			...
			messageConsumer.setMessageListener(new MessageListener() {
				...
				public void onMessage(Message message) {
					...
					try {
						...
						consumer.consumeConfigTopicWithNotifications(config);
					} catch (Exception ex) {
						workflow.reportInternalException(ex);
					} finally { 
						try {
